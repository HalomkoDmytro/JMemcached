...
import net.devstudy.jmemcashed.exception.JMemcashedConfigException;
...
import net.devstudy.jmemchashed.server.ClientSocketHandler;
...
class DefaultServerConfig implements ServerConfig {
    ...
    protected void loadApplicationProperties(String classPathResource) {
        try (InputStream in = getClassParhResourceInputStream(classPathResource)) {
            if (in == null) {
                throw new JMemcashedConfigException("Classpath resource not found: " + classPathResource);
            } else {
                applicationProperties.load(in);
            }
        } catch (IOException e) {
            throw new JMemcashedConfigException("Can't load application properties from classpath:" + classPathResource, e);
        }
    }
    ...
    public int getClearDataIntervalInMs() {
        ...
        try {
            ...
            if (clearDataIntervalInMs < 1000) {
                throw new JMemcashedConfigException("jmemcached.storage.clear.data.interval.ms should be >= 1000 ms");
            }
            return clearDataIntervalInMs;
        } catch (NumberFormatException e) {
            throw new JMemcashedConfigException("jmemcached.storage.clear.data.interval.ms should be a number", e);
        }
    }
    ...
    public int getServerPort() {
        ...
        try {
            ...
            if (port < 0 || port > 65535) {
                throw new JMemcashedConfigException("jmemcached.server.port should be between 0 and 65535");
            }
            return port;
        } catch (NumberFormatException e) {
            throw new JMemcashedConfigException("jmemcached.server.port should be a number", e);
        }
    }
    ...
    protected int getThreadCount(String propertyName) {
        ...
        try {
            ...
            if (threadCount < 1) {
                throw new JMemcashedConfigException(propertyName + " should be >= 1");
            }
            return threadCount;
        } catch (NumberFormatException e) {
            throw new JMemcashedConfigException(propertyName + " should be a number", e);
        }
    }
